"""
EDITH Cloud Node Background Daemon — v2.0

Responsibilities (HEADLESS CLOUD SERVER):
  - Nightly maintenance (ChromaDB backup, consolidation, cleanup)
  - Pre-fetch engine (weather, calendar, news, email)
  - Proactive push (disk warnings, break reminders via KDE/Telegram)
  - KDE Connect heartbeat
  - Weekly briefing generation + push
  - Self-improvement scans (ArXiv, propose upgrades)
  - Graph memory triple extraction
  - Health checks + monitoring
  - Weekly provider router tuning
  - Systemd watchdog integration

LOCAL TASKS REMOVED:
  - Subprocess management (chat_server.py, wake_listener.py)
...
from config import get_logger, EDITH_PATH, MEMORY_DB_PATH, KDE_DEVICE_ID

log = get_logger("background_daemon_cloud")
...
def _graceful_shutdown(signum, frame):
    """Handle SIGTERM/SIGINT: flush data, exit cleanly."""
...
def _setup_schedule():
    ...
    # KDE heartbeat is local-only; cloud nodes do not have KDE Connect devices.
    ...
    schedule.every(30).minutes.do(_validate_all)
...
# ──────────────────────────────────────────────
# Watchdog Loop (Cloud version — no subprocess monitoring)
# ──────────────────────────────────────────────
def _watchdog_loop():
    """Main loop: send watchdog keepalive (no subprocess monitoring)."""
    while not _shutdown_event.is_set():
...
if __name__ == "__main__":
    log.info("=" * 50)
    log.info("EDITH Cloud Node Daemon starting...")
    log.info("=" * 50)
    ...
    _sd_notify("READY=1")
    log.info("Cloud node ready. Scheduler + watchdog active.")
